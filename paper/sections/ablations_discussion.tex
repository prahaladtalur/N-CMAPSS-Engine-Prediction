...
The multiseed runs (Table~\ref{tab:multiseed}) further bound the single-run estimates: MSTCN's mean RMSE over three seeds is 8.140 ($\pm0.74$, T5) and WaveNet's over two seeds is 8.238 ($\pm1.44$, T6). At this sample size the means are not meaningfully separable, and both lie above their respective best-run values (T3: 7.604; T1: 6.523), so the FD1 gap between WaveNet and MSTCN in the best-run table should be interpreted alongside seed variability rather than as a stable architecture ordering.
