...
def main():
    ...
    parser.add_argument(
        "--model", default="mstcn", help="Model to tune (for quick_wins and hyperparams)"
    )
    parser.add_argument("--fd", type=int, default=1, help="N-CMAPSS FD split to use")
    parser.add_argument(
        "--epochs", type=int, default=None, help="Override default epochs per suite"
    )
    parser.add_argument("--project", default="sota-tuning", help="Weights & Biases project name")
    parser.add_argument(
        "--models",
        nargs="+",
        default=None,
    ...
    parser.add_argument(
        "--offline", action="store_true", help="Run W&B in offline mode for local benchmarking"
    )
